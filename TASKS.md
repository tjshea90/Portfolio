...
- [x] Give `tradePlan`'s decline a REASON, not just a null — refactored into internal
      `planInternal` returning `Pair<TradePlan?, String>`, with `tradePlan()` and a new
      `tradePlanDeclineReason()` as thin wrappers so none of the 33 existing direct
      callers/tests changed shape. Verified: `compileDebugKotlin`/`compileDebugUnitTestKotlin`
      clean, every pre-existing `tradePlan`-based test still passes unmodified.
- [x] New `ResearchRow.planReason` (bookkeeping/UI text, same treatment as
      ...
      `entryPrice` already uses), cleared the instant a real plan returns. 3 new tests
      (`ResearchPriceFillTest`): blank on a single declined tick, set on a confirmed one, and
      cleared the instant a real plan comes back.
- [x] UI: when `entryPrice <= 0` and `planReason` is non-blank, show it — muted
      `onSurfaceVariant`, not red. Both `ResearchCard` (the list) and `DayTradingPlanContent`
      (the tabbed detail view, replacing the previous silent no-`else` gap).
- [x] A one-time FULL sweep on Day Trading tab open: `enrichDayTradingVisible`'s first call
      after a rebuild (`dayTradingSweepDone` gate) now fetches the WHOLE section instead of
      just the paged window, then calls the new pure `sortDayTradingForActionability`:
      actionable rows (real entry/stop/target, app's or Claude's) first, ranked by
      `blendedScore` within each group, stable so ties keep the screener's own order. 4 new
      tests pin this directly (actionable-over-declined regardless of score, ranking within a
      group, stability on a tie, a Claude-authored plan with no likelihood/confidence still
      outranking every decline).
- [x] The ordinary 30-second tick after the sweep goes back to the paged `shown` window and
      never re-sorts, exactly as before this round — confirmed by code path (`sweeping` is
      only true once per rebuild) and by the doc header rewritten to say so precisely rather
      than flatly (a stale claim caught in self-review, since the function now genuinely does
      sort on the one call that matters).
- [x] UI-bug sweep: `LazyColumn` is keyed by `"${section.key}_${it.symbol}"`, not index, so
      the one-time resort cannot desync per-row Compose state (chart, loading) or crash on
      reorder. The v7.21 hysteresis fix is untouched and still the only thing that clears
      levels on an ordinary tick. Traced the "Load more" path: since the sweep now covers all
      40 rows up front, paging in more rows no longer reveals never-enriched blank cards
      waiting on the next 30s tick — a real secondary improvement, not just parity. Found and
      fixed one self-introduced doc bug (a dead `[sweepAllDayTrading]` reference — that
      function was never created; the sweep lives inside `enrichDayTradingVisible` itself)
      and one stale doc claim (see above) before they could mislead a future session.
- [x] Tests: 10 new (3 decline-reason in `DayTradingTest`, 3 `planReason` threading + 4
      sort in `ResearchPriceFillTest`). Full Gradle suite green: 1075 tests, 0 failures
