...
- [ ] Verify + fix every finding (agents can be wrong - check each against the code), with
    ...
    - M: [ ] C-2 5D "truncated" Fridays  [ ] C-3 vs-SPY All range different starts  [ ] C-4 zoom
      ...
      plans dropped (MarketClock.sessionFor; planStillForSession + answerIsCurrent extended; +2 tests)  [ ] D-4 no plans after rebuild until next tick  [x] D-5 forming first bar /
      opening range used as trigger (or5 only once a 09:35 bar exists; OR levels only when complete;
      +2 tests; DayTradingTest/SoundnessTest fixtures now say orComplete explicitly)  [x] L-2 return during unwinding pass drops refresh (guard on quoteJob.isActive + quoteGen so a
      ...
      sameTradingDay; test)  [x] S-8 ETF
      dedupe merges different products (strategy words tilt; munis by state/HY/band; HY banded;
...
- [x] Light tests, ship, post link (same standing instruction as 2026-09-23c)
      SHIPPED v7.39/96: run 35894509527 green on attempt 2 (attempt 1 died in emulator SDK download, infra), Release published 17:35 UTC, BUILDLOG recorded, link posted.
