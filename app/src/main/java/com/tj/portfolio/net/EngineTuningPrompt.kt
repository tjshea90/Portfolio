...
""".trimStart())
        ...
        sb.append("- Switching a per-setup override OFF moves that setup to the global value - limited to the same step, measured from the override to the global (or, when the global is off too, to the lenient end of its range).\n")
        sb.append("- The whole answer must leave a consistent engine: `time.lastEntryMinutes` at least ${EngineTuning.MIN_ENTRY_TO_FLAT_MINUTES} more than `time.flatBeforeCloseMinutes`, no stop floor above its ceiling (globally or for a setup), at least one setup on, and at least ${EngineTuning.MIN_ENTRY_WINDOW_MINUTES} minutes a day to start a trade. This is judged on the answer as a whole (moving a floor and its ceiling together is fine in any order); when it fails, the last listed change that causes it is refused.\n")
