# RESUME — READ THIS FIRST  (round 66, saved 2026-09-10 13:13:28 UTC)
...
## 5. Open findings — 6 still open, 68 fixed
...
- [ ] CRX2 (high) MERGE restore matches each incoming row against rows THIS restore just inserted, so N genuinely identical transactions in a backup collapse to one - silent data loss in the one path that exists to recover lost transactions
