# RESUME — READ THIS FIRST  (round 64, saved 2026-09-09 02:01:23 UTC)
...
## 5. Open findings — 6 still open, 32 fixed
...
- [ ] J02 (high) Readout colour comes from the whole series while its value comes from the window: a negative change printed in green on a zoomed chart
- [ ] J03 (med) clipToWindow(pad=false)'s straddle fallback picks lo..lo+1 instead of the straddling pair, so 'inside' and insideIndices describe different points; a single in-window point is also widened to two
