...
### R2T-3 (H): the "By opening bar direction" table files every plan shown 09:30-09:34 under "did not close up" (wrong data for the one switch it exists to tune)
...
- **Why it is not a corner case:** the log keeps each symbol's FIRST loggable plan of the day (`INSERT OR IGNORE`, `dtLoggedToday`), and the first live tick logs every shown row that has a waiting plan. So whenever Tj has the tab open at the bell, most of that day's rows are logged in the first minutes, and all of them get `obb:false`. The "closed up" group then holds only the rows that first became loggable later in the day, which is a different population.
...
- **Failing scenario:** v3 = apply A (sets `stop.minRiskAtrs`), v4 = apply B (sets `target.capR`). Two taps on the dialog's "Undo" are both dispatched before the recomposition that removes the dialog. That is unlikely at 60 fps, but possible when the main thread is janky, or with an accessibility double-activation. B is undone (v5), then A is undone (v6), leaving the original engine after one confirmation.
